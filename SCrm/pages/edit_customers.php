...
$this_page = plugin_page('edit_customers');
$this_page_create = plugin_page('edit_customer_record') . '&action=create';
$this_page_edit = plugin_page('edit_customer_record') . '&action=edit';
...
$list_query = DAOCustomer::get_list(
	$p_sort_field,
	$p_sort_dir,
	$p_hideinactive,
	$p_search,
	...
	<div class="widget-box widget-color-blue2">
		...
		<div class="widget-header widget-header-small">
			<h4 class="widget-title lighter">
				...
				<?php echo plugin_lang_get('main_menu_customers');?>	
				<span class="badge"><?php echo plugin_lang_get('global_cmd_total') . ":" .  $resStat['customer_count'];?></span>
			</h4>
		</div>
								...
								<?php
									if (access_has_global_level(config_get(SCrmPlugin::CFG_KEY_MANAGE_TABLES_TRESHOLD)))
									{
										echo "<a class=\"btn btn-primary btn-white btn-round btn-sm\" href=\"{$this_page_create}\">";
											echo plugin_lang_get('edit_customers_page_cmd_add_customer');
				...
				<div class="table-responsive">
					...
					<thead>
							<tr>
								...
								<th style="width:20%;">
									<a href="<?php echo $this_page . '&sort_field=customer_name&amp;sort_dir=' . $sort_dir_new . '&amp;hideinactive=' . $p_hideinactive . '&amp;search=' . urlencode($p_search); ?> ">
										<?php echo plugin_lang_get('table_customer_col_customer_name');?>
									</a>
									<?php 
										if ($p_sort_field == 'customer_name') {
											echo $sort_icon;
										}
									?>
								</th>
								<th style="width:15%;">
									<a href="<?php echo $this_page . '&sort_field=ident_number&amp;sort_dir=' . $sort_dir_new . '&amp;hideinactive=' . $p_hideinactive . '&amp;search=' . urlencode($p_search); ?> ">
										<?php echo plugin_lang_get('table_customer_col_ident_number');?>
									</a>
									<?php 
										if ($p_sort_field == 'ident_number') {
											echo $sort_icon;
										}
									?>
								</th>
								<th style="width:15%;">
									<a href="<?php echo $this_page . '&sort_field=email&amp;sort_dir=' . $sort_dir_new . '&amp;hideinactive=' . $p_hideinactive . '&amp;search=' . urlencode($p_search); ?> ">
										<?php echo plugin_lang_get('table_customer_col_email');?>
									</a>
									<?php 
										if ($p_sort_field == 'email') {
											echo $sort_icon;
										}
									?>
								</th>
								<th style="width:15%;">
									<a href="<?php echo $this_page . '&sort_field=phone&amp;sort_dir=' . $sort_dir_new . '&amp;hideinactive=' . $p_hideinactive . '&amp;search=' . urlencode($p_search); ?> ">
										<?php echo plugin_lang_get('table_customer_col_phone');?>
									</a>
									<?php 
										if ($p_sort_field == 'phone') {
											echo $sort_icon;
										}
									?>
								</th>
								<th style="width:15%;">
									<a href="<?php echo $this_page . '&sort_field=group_name&amp;sort_dir=' . $sort_dir_new . '&amp;hideinactive=' . $p_hideinactive . '&amp;search=' . urlencode($p_search); ?> ">
										<?php echo plugin_lang_get('table_group_col_group_name');?>
									</a>
									<?php 
										if ($p_sort_field == 'group_name') {
											echo $sort_icon;
										}
									?>
								</th>
						...
						<tbody>
							<?php 
								...
								$page_callback_link_url = plugin_page('edit_customers',true) .urlencode( 
								...
								while( $row = db_fetch_array( $list_query ) ) 
								{
									...
									echo "<tr>" .
										...
										"<td> {$row_link}" . $row['customer_name'] . "</a></td>" .
										"<td> {$row_link}" . $row['ident_number'] . "</a></td>" .
