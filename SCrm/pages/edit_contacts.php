...
$this_page = plugin_page('edit_contacts');
$this_page_create = plugin_page('edit_contact_record') . '&action=create';
$this_page_edit = plugin_page('edit_contact_record') . '&action=edit';
...
$list_query = DAOContact::get_list(
	$p_sort_field,
	$p_sort_dir,
	$p_hideinactive,
	$p_search,
	...
	<div class="widget-box widget-color-blue2">
		...
		<div class="widget-header widget-header-small">
			<h4 class="widget-title lighter">
				...
				<?php echo plugin_lang_get('main_menu_contacts');?>	
				<span class="badge"><?php echo plugin_lang_get('global_cmd_total') . ":" .  $resStat['contact_count'];?></span>
			</h4>
		</div>
								...
								<?php
									if (access_has_global_level(config_get(SCrmPlugin::CFG_KEY_MANAGE_TABLES_TRESHOLD)))
									{
										echo "<a class=\"btn btn-primary btn-white btn-round btn-sm\" href=\"{$this_page_create}\">";
											echo plugin_lang_get('edit_contacts_page_cmd_add_contact');
					...
					<table class="table table-striped table-bordered table-condensed table-hover">
						<thead>
							<tr>
								...
								<th style="width:20%;">
									<a href="<?php echo $this_page . '&sort_field=first_name&amp;sort_dir=' . $sort_dir_new . '&amp;hideinactive=' . $p_hideinactive . '&amp;search=' . urlencode($p_search); ?> ">
										<?php echo plugin_lang_get('table_contact_col_first_name');?>
									</a>
									<?php 
										if ($p_sort_field == 'first_name') {
											echo $sort_icon;
										}
									?>
								</th>
								<th style="width:20%;">
									<a href="<?php echo $this_page . '&sort_field=second_name&amp;sort_dir=' . $sort_dir_new . '&amp;hideinactive=' . $p_hideinactive . '&amp;search=' . urlencode($p_search); ?> ">
										<?php echo plugin_lang_get('table_contact_col_second_name');?>
									</a>
									<?php 
										if ($p_sort_field == 'second_name') {
											echo $sort_icon;
										}
									?>
								</th>
						...
						<tbody>
							<?php 
								...
								$page_callback_link_url = plugin_page('edit_contacts',true) .urlencode( 
								...
								while( $row = db_fetch_array( $list_query ) ) 
								{
									...
									echo "<tr>" .
										...
										"<td> {$row_link}" . $row['first_name'] . "</a></td>" .
										"<td> {$row_link}" . $row['second_name'] . "</a></td>" .
